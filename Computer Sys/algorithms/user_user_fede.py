...
def calculate_user_collaborative_matrix(train_clean_data, test_clean_data):
    ...
    return user_sims.filter(lambda x: x[0] in test_users).map(
        lambda p: topNRecommendations(p[0],p[1],uib.value,-1))
